...
class ReportCards extends CRUD
{
    ...
    protected function getRequiredUpdateData() {
        return ['athletes_id', 'secondary_coach_id', 'levels_id', 'comment', 'approved', 'status', 'session', 'day_of_week'];
    }
    
    protected function getUpdateSQL() {
        $date = $this->getCurrentDateTime();
        return 'UPDATE '.$this->getTableName().
                " SET athletes_id = :athletes_id, secondary_coach_id = :secondary_coach_id, levels_id = :levels_id, comment = :comment, approved = :approved, status = :status, session = :session, day_of_week = :day_of_week, updated_date = '$date'".
                ' WHERE id = :id';
    }
